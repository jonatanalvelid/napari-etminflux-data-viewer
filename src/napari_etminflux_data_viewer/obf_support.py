...
omas_data_types = {
    0x00000001: np.uint8,
    0x00000002: np.int8,
    0x00000004: np.uint16,
    0x00000008: np.int16,
    0x00000010: np.uint32,
    0x00000020: np.int32,
    0x00000040: np.float32,
    0x00000080: np.float64,
    0x00001000: np.uint64,
    0x00002000: np.int64,
    0x00010000: np.bool_,
}
...
class File:
        ...
        try:
            ...
            with open(file_path, "rb") as file:
                ...
                if self.format_version >= 2:
                    ...
                    file_meta_data_pos = long_unpack(
                        self._file.read(long_len)
                    )[0]
                ...
                while next_stack_pos != 0:
                    ...
                    if value not in omas_data_types:
                        raise RuntimeError(f"Unsupported data type {value}.")
                    else:
                    ...
                    while (
                        dimensionality > 1
                        and stack.shape[dimensionality - 1] == 1
                    ):
                        dimensionality -= 1
                    ...
                    if stack.format_version >= 1:
                        ...
                        footer["has_col_positions"] = values[1:15][
                            : stack.rank
                        ]
                        ...
                        if stack.format_version >= 2:
                            ...
                            for i in range(stack.rank):
                                stack.si_dimensions.append(
                                    SIUnit(values[i * 19 : (i + 1) * 19])
                                )
                        ...
                        # read label strings
                        stack.labels = [
                            self._read_string() for _ in range(stack.rank)
                        ]

                        # read col positions
                        if "has_col_positions" in footer:
                            stack.col_positions = {}
                            for axis, has_them in enumerate(
                                footer["has_col_positions"]
                            ):
                                if has_them:
                                    # read doubles as positions
                                    fmt = f"<{stack.shape[axis]}d"
                        ...
                        if "metadata_length" in footer:
                            stack.metadata = self._read_string(
    ...
    def _read_string(self, length: int = None) -> str:
        ...
        fmt = f"<{length}s"
        data = self._file.read(struct.calcsize(fmt))
        string = struct.unpack_from(fmt, data)[
            0
    ...
    def _read_stack(self, stack: Stack):
        ...
        try:
            ...
            data = []
            if (
                stack._compression_type == 1
                and stack.footer["samples_written"] > 0
            ):
                ...
                if "num_chunk_positions" in stack.footer:
                    ...
                    bytes_written = min(
                        stack.footer["samples_written"]
                        * stack.data_type().itemsize
                        + 16,
                        stack.footer["chunk_positions"][-1][0]
                        + stack.footer["stack_end_used_disk"]
                        - stack.footer["chunk_positions"][-1][1],
                    )  # this is a bit heuristic and not documented but I don't want to read too much
            ...
            while pos < bytes_written:
                ...
                if (idx < len(stack.footer["chunk_positions"])) and (
                    pos + bytes_to_read
                    > stack.footer["chunk_positions"][idx][0]
                ):  # chunk_positions[0] = logical offset, [1] = file offset
                    bytes_to_read = (
                        stack.footer["chunk_positions"][idx][0] - pos
                    )
                    seek_pos = (
                        stack.footer["chunk_positions"][idx][1]
                        + stack._data_pos
                    )
                    idx += 1
                if bytes_to_read > 0:
            ...
            data = b"".join(
                data
            )  # is there a more efficient way to concatenate byte arrays?
...
class Stack:
    ...
    def __getattr__(self, name: str):
        ...
        if name == "pixel_sizes":
            ...
            pixel_sizes = [
                length / n if n > 0 else math.nan
                for length, n in zip(self.lengths, self.shape, strict=True)
            ]
            return pixel_sizes
        elif name == "data":
...
class SIUnit:
    ...
    def __str__(self) -> str:
        ...
        for exponent, unit in zip(
            self.exponents, SIUnit.unitnames, strict=True
        ):
